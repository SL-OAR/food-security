...
**Document produced** in `R` `Markdown` using `emmeans`, `glue`, `ggraph`, `ggtext`, `huxtable`, `igraph`, `janitor`, `readxl`, `swasi`, `tidytext`, `tidyverse` (`dplyr`, `ggplot2`, `forcats`, `purrr`, `stringr`), `tm`, `udpipe`, `viridis`, and `wordcloud2` packages.
...
Food security was measured using the **10-item** United States Department of Agriculture Food Security Module.

### Current Results -- `r current_cy` {.tabset .tabset-pills .tabset-fade}

#### Disability/Neurodivergence

Students who have disabilities or are neurodivergent were significantly less likely to be food secure than were those who do not disabilities or are not neurodivergent (*b* = `r stat_2(sp_models$ay_2324$d$coefficients[2,1])`, *z*(`r big_n(sp_models$ay_2324$d$df.residual)`) = `r stat_2(sp_models$ay_2324$d$coefficients[2,3])`, *p* < 0.001).

```{r, out.width = "100%", echo = FALSE, warning = FALSE}
oe_bd_plots$ay_2324$d
```

Note: *n* = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&!is.na(fs$giso_desc)])))` (*n*~*2SLGBTQIA3+*~ = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$d_desc_bin_1=="Disabled/Neurodivergent"])))`, *n*~*Not 2SLGBTQIA3+*~ = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$d_desc_bin_1=="Not Disabled/Neurodivergent"])))`). Regressions were computed using the `glm` function with gender identity/sexuality dummy coded as *Not Disabled/Neurodivergent* = 0, *Disabled/Neurodivergent* = 1.

#### Gender Identity/Sexuality

2SLGBTQIA3+ students were significantly less likely to be food secure than were those who are not 2SLGBTQIA3+ (*b* = `r stat_2(sp_models$ay_2324$giso$coefficients[2,1])`, *z*(`r big_n(sp_models$ay_2324$giso$df.residual)`) = `r stat_2(sp_models$ay_2324$giso$coefficients[2,3])`, *p* = `r stat_2(sp_models$ay_2324$giso$coefficients[2,4])`).
...
Asian students were significantly more likely to be food secure than was the average student (*b* = `r stat_2(sp_models$ay_2324$re$coefficients[2,1])`, *z*(`r big_n(sp_models$ay_2324$re$df.residual)`) = `r stat_2(sp_models$ay_2324$re$coefficients[2,3])`, *p* = `r stat_2(sp_models$ay_2324$re$coefficients[2,4])`) and Latin* students were significantly less likely to be food secure than was the average student (*b* = `r stat_2(sp_models$ay_2324$re$coefficients[3,1])`, *z*(`r big_n(sp_models$ay_2324$re$df.residual)`) = `r stat_2(sp_models$ay_2324$re$coefficients[3,3])`, *p* < 0.001. Black/Indigenous (*b* = `r stat_2(sp_models$ay_2324$re$coefficients[5,1])`, *z*(`r big_n(sp_models$ay_2324$re$df.residual)`) = `r stat_2(sp_models$ay_2324$re$coefficients[5,3])`, *p* = `r stat_2(sp_models$ay_2324$re$coefficients[5,4])`) and Multiracial/ethnic (*b* = `r stat_2(sp_models$ay_2324$re$coefficients[4,1])`, *z*(`r big_n(sp_models$ay_2324$re$df.residual)`) = `r stat_2(sp_models$ay_2324$re$coefficients[4,3])`, *p* = `r stat_2(sp_models$ay_2324$re$coefficients[4,4])`) students did not differ significantly on food security compared to the average student. However, small group size may have obscured a real difference. The odds of Black/Indigenous students being food *insecure* were 40% greater than those of the average student (*OR* = `r stat_2(1/exp(sp_models$ay_2324$re$coefficients[5,1]))`).

```{r, out.width = "100%", echo = FALSE, warning = FALSE}
oe_bd_plots$ay_2324$re
```

Note: *n* = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&!is.na(fs$re_reduced)])))` (*n*~*Asian*~ = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$re_reduced=="Asian"])))`, _n_~_Latin_\*~ = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$re_reduced=="Latin*"])))`, *n*~*Black/Indigenous*~ = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$re_reduced=="Black/Indigenous"])))` [`r round(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$re_desc=="Black"]))/sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$re_reduced=="Black/Indigenous"]))*100,0)`% Black], *n*~*Multiracial/ethnic*~ = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$re_reduced=="Multiracial/ethnic"])))`, *n*~*White*~ = `r big_n(sum(!is.na(fs$fs_desc_bin[fs$ayear==2324&fs$re_reduced=="White"])))`). Regressions were computed using the `glm` function with race/ethnicity being weighted effects coded ([J. Cohen et al., 2003](https://psycnet.apa.org/record/2002-18109-000){target="_blank"}) with *White* as the base category, which means that *Asian*, *Black/Indigenous*, _Latin_\*, and *Multiracial/ethnic* were compared to "the average student," weighted by group size.

#### Socioeconomic Status

Students of lower socioeconomic status were significantly less likely to be food secure (*b* = `r stat_2(sp_models$ay_2324$ses$coefficients[2,1])`, *z*(`r big_n(sp_models$ay_2324$ses$df.residual)`) = `r stat_2(sp_models$ay_2324$ses$coefficients[2,3])`, *p* < 0.001).
...
Transfer students were significantly less likely to be food secure (*b* = `r stat_2(sp_models$ay_2324$ts$coefficients[2,1])`, *z*(`r big_n(sp_models$ay_2324$ts$df.residual)`) = `r stat_2(sp_models$ay_2324$ts$coefficients[2,3])`, *p* = `r round(sp_models$ay_2324$ts$coefficients[2,4],3)`) than were first-time students.
...
When demographics predictors of food security that were significant or at least substantial (disability/neurodivergence, gender identity/sexuality, race/ethnicity, socioeconomic status, transfer status) were all entered into one model, **disability/neurodivergence** (*b* = `r stat_2(mp_models$ay_2324$coefficients[9,1])`, *z*(`r big_n(mp_models$ay_2324$df.residual)`) = `r stat_2(mp_models$ay_2324$coefficients[9,3])`, *p* < 0.001), **race/ethnicity**, specifically the comparison of Asian students to the average student (*b* = `r stat_2(mp_models$ay_2324$coefficients[3,1])`, *z*(`r big_n(mp_models$ay_2324$df.residual)`) = `r stat_2(mp_models$ay_2324$coefficients[3,3])`, *p* = `r stat_2(mp_models$ay_2324$coefficients[3,4])`), and **socioeconomic status** (*b* = `r stat_2(mp_models$ay_2324$coefficients[7,1])`, *z*(`r big_n(mp_models$ay_2324$df.residual)`) = `r stat_2(mp_models$ay_2324$coefficients[7,3])`, *p* < 0.001), **remained significant**. None of the nonsignificant predictors were of substantial size.

___

Note: Predictor variables were disability/neurodivergence (dummy coded -- *Not Disabled/Neurodivergent* = 0, *Disabled/Neurodivergent* = 1) gender identity/sexuality (dummy coded -- *Not LGBTQIA+* = 0, *LGBTQIA+* = 1), race/ethnicity (weighted effects coded [J. Cohen et al., 2003](https://psycnet.apa.org/record/2002-18109-000){target="_blank"} with *White* as the base category, which means that which means that *Asian*, *Black/Indigenous*, _Latin_\*, and *Multiracial/ethnic* are compared to "the average student," weighted by group size), socioeconomic status (ordinal scale: 0-4 indicators of lower socioeconomic status; see [Supplemental Method](#supmeth) for details), and transfer status (dummy coded -- *First-Time* = 0, *Transfer* = 1).
...
### `r cy_2023` {.tabset .tabset-pills .tabset-fade #bn}
